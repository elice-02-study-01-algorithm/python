# 미완성 코드예요!
# 짝수 번째에 0이 있고 동시에 해당 번호의 자식 노드들이 0이 아니면 False이다
# 0일 때 BFS를 진행해서 전부 0인지 검사하려고 짜다가 포기
...
def makeDict(binaryNumber):
    nodeNumber = len(binaryNumber)
    level = 1
    while True:
        if nodeNumber == 1:
            break
            ...
            level += 1
    print('root', level)
    levelFirstNodes = [2**i for i in range(level)]
    resultDict = {}
    for i in range(len(levelFirstNodes)):
        if i == 1:
            pass
        elif i == len(levelFirstNodes)-1:
            resultDict[levelFirstNodes[i]] = [levelFirstNodes[i]//2, levelFirstNodes[i]+levelFirstNodes[i]//2]
        else:
            for _ in range((level-1)**2):
                resultDict[levelFirstNodes[i]] = [levelFirstNodes[i]-levelFirstNodes[i]//2, levelFirstNodes[i]+levelFirstNodes[i]//2]
        
        level -= 1

    print(levelFirstNodes)
    return resultDict
3
...
def solution(numbers):
    ...
    for number in numbers:
        print('make', makeDict(makeBinaryTree(number)))
